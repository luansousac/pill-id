...
irx_train, irx_test, iry_train, iry_test = train_test_split(irx, iry,
                                                            test_size=0.2)

digits = load_digits()
dgx, dgy = stdsc.fit_transform(digits.data/16.0), digits.target
dgx_train, dgx_test, dgy_train, dgy_test = train_test_split(dgx, dgy,
                                                            test_size=0.2)

diabetes = load_diabetes()
dbx, dby = stdsc.fit_transform(diabetes.data), diabetes.target
dbx_train, dbx_test, dby_train, dby_test = train_test_split(dbx, dby,
                                                            test_size=0.2)

mrx, mry = make_regression(n_samples=2000, n_targets=4)
mrx_train, mrx_test, mry_train, mry_test = train_test_split(mrx, mry,
                                                            test_size=0.2)

xtoy, ytoy = make_toy()
xtoy, ytoy = stdsc.fit_transform(xtoy), stdsc.fit_transform(ytoy)
xtoy_train, xtoy_test, ytoy_train, ytoy_test = train_test_split(xtoy, ytoy,
                                                                test_size=0.2)
plt.plot(xtoy, ytoy)
...
elmr = pipeline.Pipeline([('rhl', RandomLayer(random_state=0,
...
tanh_rhl = MLPRandomLayer(n_hidden=100, activation_func=powtanh_xfer,
                          activation_args={'power': 3.0})
elmc_tanh = GenELMClassifier(hidden_layer=tanh_rhl)
elmc_tanh.fit(dgx_train, dgy_train)
print(elmc_tanh.score(dgx_train, dgy_train), elmc_tanh.score(dgx_test,
                                                             dgy_test))

# <codecell>

rbf_rhl = RBFRandomLayer(n_hidden=100, rbf_width=0.01)
tr, ts = res_dist(dgx, dgy, GenELMClassifier(hidden_layer=rbf_rhl),
...
tr, ts = res_dist(dbx, dby, RandomForestRegressor(n_estimators=15),
                  n_runs=100, random_state=0)
...
elmc = ELMClassifier(n_hidden=1000, activation_func='gaussian', alpha=0.0,
                     random_state=0)
...
elmc = ELMClassifier(n_hidden=500, activation_func='hardlim', alpha=1.0,
                     random_state=0)
elmc.fit(dgx_train, dgy_train)
print(elmc.score(dgx_train, dgy_train), elmc.score(dgx_test, dgy_test))
